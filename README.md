...
~23 400 строк, 18 глав и 4 приложения, 225 разделов с собственным коротким якорем `#N.M`. Все примеры проверены на CPython 3.12+.
...
| XIII | [Тестирование](book/13-testing.md) | assert в байт-коде, unittest/mock изнутри, doctest, pytest (fixtures, assertion rewriting), coverage.py, sys.settrace и sys.monitoring (PEP 669), тесты самого CPython, диагностические режимы для CI |
| XIV | [Упаковка](book/14-packaging.md) | importlib.metadata, анатомия dist-info (METADATA/RECORD/WHEEL), pyproject.toml (PEP 621), build-бэкенды и изоляция PEP 517, sdist vs wheel, теги wheel-имени, pip изнутри (резолвер, кэши, генерация скриптов), editable-механика (.pth и finder), PEP 508/440 (спекифаеры, маркеры), entry points и плагины, публикация на PyPI, zipapp + бенчмарки pip/uv |
| XV | [От .py к байт-коду: компиляция и VM](book/15-bytecode-vm.md) | компиляция vs интерпретация, формат `.pyc` (PEP 552), PEG→AST→байт-код, стековая машина, специализирующий интерпретатор (PEP 659), GIL, JIT |
...
Полное кликабельное оглавление по всем 225 разделам — на сайте: **<https://n3xumc0r3.github.io/>**
