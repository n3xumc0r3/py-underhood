...
| XVIII | [Упаковка](book/18-packaging.md) | importlib.metadata, анатомия dist-info (METADATA/RECORD/WHEEL), pyproject.toml (PEP 621), build-бэкенды и изоляция PEP 517, sdist vs wheel, теги wheel-имени, pip изнутри (резолвер, кэши, генерация скриптов), editable-механика (.pth и finder), PEP 508/440 (спекифаеры, маркеры), entry points и плагины, публикация на PyPI, zipapp + бенчмарки pip/uv |
