...
**Правильная альтернатива** для вычисления математики от пользователя — `ast.literal_eval` (только литералы: числа, строки, кортежи, списки, dict) или специализированные парсеры:
...
- Возвращает **имена**, не значения. Для значений используй `getattr(obj, name)` в цикле или `inspect.getmembers()`.
