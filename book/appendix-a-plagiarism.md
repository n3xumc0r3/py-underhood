...
def winnow(text: str, k: int = 5, w: int = 4) -> list:
    ...
    Соотношение w = k - 1 (как здесь) гарантирует по теореме Schleimer et al. 2003,
    что любая общая подстрока длины ≥ 2k-1 породит хотя бы один общий отпечаток."""
...
Считается количество независимых путей выполнения через граф потока управления. Базовая формула McCabe: `M = E − N + 2P`, где `E` — рёбра, `N` — узлы, `P` — компоненты связности. Часто упрощают до `M = количество if + количество for/while + количество case в match/case + 1`.
