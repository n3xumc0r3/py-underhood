...
for x in cycle([1, 2, 3]):
    print(x, end=' ')
    if x == 3: break   # напечатает "1 2 3 " (с хвостом пробела) и прервётся после первого круга
...
# Поиск локальных максимумов — нужны тройки, поэтому pairwise (дающий пары) тут не помогает
peaks = [b for a, b, c in zip(data, data[1:], data[2:]) if a < b > c]
# 15, 20 — через срезы; вариант без материализации срезов — три pairwise со смещением
```
...
⚠️ `namedtuple` неизменяемый и хешируемый (если все поля хешируемы — как обычный `tuple`; `Point(1, [2])` будет `TypeError: unhashable type: 'list'`) — можно использовать как ключ в словаре. `typing.NamedTuple` — тот же namedtuple с аннотациями, он тоже НЕизменяемый. Если нужна мутабельность — берите `@dataclass`.
