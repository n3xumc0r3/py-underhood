...
В туториалах `with` открывает файлы. На самом деле это протокол из двух методов, поверх которого построены блокировки, транзакции, подмена stdout, точечное подавление исключений и стеки завершающих действий. Часть идёт от механики (`__enter__`/`__exit__`) через собственные менеджеры к `contextlib` и его менее известным обитателям — и каждый раз показывает, что происходит при выходе из блока, включая аварийный.
