...
Эта часть — про то, как узнать, **как именно** запущен интерпретатор и что происходит в процессе: `sys.flags` и `sys._xoptions` (10.1–10.2), `PYTHON*`-переменные (10.3), лимиты ресурсов и `/proc` (10.5–10.6), audit hooks (10.7) и faulthandler (10.8). Отсюда же растут CLI-темы 11.32 и диагностические режимы Части XIII.
...
- `PYTHON_BASIC_REPL` — `1` возвращает до-3.13 REPL без `_pyrepl` (см. 17.4).
