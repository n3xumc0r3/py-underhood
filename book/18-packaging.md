...
| **wheel / sdist** | два формата артефактов дистрибуции (18.5) | `demo_pkg_hood-0.1.0-py3-none-any.whl` |
...
| `requires(name)` | список строк-зависимостей (PEP 508, 18.9) | `Requires-Dist:` в METADATA |
| `files(name)` | список `PackagePath` | `RECORD` |
| `entry_points(group=...)` | `EntryPoints` | `entry_points.txt` (18.10) |
...
## 18.2. Анатомия `dist-info`: паспорт установленного пакета { #18.2 }

Всё, что pip «знает» об установленной дистрибуции, лежит в каталоге `<нормализованное_имя>-<версия>.dist-info` рядом с кодом в `site-packages` (11.31). Реальный пример из живого окружения — `packaging-26.0.dist-info`, собранный бэкендом flit и установленный uv:
...
| `entry_points.txt` | INI-секции групп точек входа | `entry_points()` (18.10) |
| `top_level.txt` | корневые импорт-имена | legacy-инструменты |

Формат `RECORD` — сердце безопасного удаления (внутри wheel того же демо, что соберём в 18.4):
...
Каждая строка — «путь, хеш, размер». Две детали, которые важно видеть: **сам RECORD записан с пустыми хешем и размером** (он не может хешировать сам себя), а пути у файлов вне `site-packages` — **относительные** (18.7 покажет `../../bin/demo-hood`). `pip uninstall` просто читает этот список и удаляет файлы построчно — потому удаление не трогает ничего, чего нет в RECORD.
...
### Что dist-info не содержит { #18.2-net }
...
## 18.3. `pyproject.toml`: PEP 621 { #18.3 }
...
name = "demo-pkg-hood"
version = "0.1.0"
description = "Демо для Части XVIII"
readme = "README.md"
requires-python = ">=3.10"
license = "MIT"                      # SPDX-выражение (PEP 639)
authors = [{name = "Author"}]
dependencies = [                     # runtime-зависимости (PEP 508, см. 18.9)
    "requests>=2.25",
...
[project.scripts]                    # консольные команды (18.10)
...
| `version` | str | PEP 440 (18.9); можно вынести в `dynamic` |
...
⚠️ `pyproject.toml` читает и pip (для `requires` из `[build-system]`), и бэкенд, и линтеры (Часть XII), но **не Python**: `import pyproject` невозможен, runtime-код не должен опираться на этот файл — версия живёт в метаданных (18.1), а не в исходниках.

## 18.4. Build-бэкенды и PEP 517 { #18.4 }
...
pip install .
   ...
   │
   └─ установка полученного wheel (18.7)
```
...
## 18.5. sdist и wheel: два формата артефактов { #18.5 }
...
**wheel** (`.whl`) — уже **готовый к установке** zip-архив (анатомия — 18.6). Ключевое отличие: wheel строит **автор пакета** (или CI), а пользователю остаётся только распаковать. Ставить wheel — это не «быстрее sdist», это качественно другой сценарий: не нужен компилятор C, не запускается чужой код сборки (18.7 о границах этого обещания), не нужна сеть для зависимостей сборки.

| | sdist | wheel |
|---|---|---|
| Формат | `.tar.gz` (PEP 625) | zip `.whl` со строгим именем (18.6) |
...
| abi-тег | `none`, `cp312`, `abi3` | `none` — чистый Python; `cp312` — своя ABI; `abi3` — stable ABI (17.9) |
...
- **`RECORD` внутри wheel не содержит `.pyc`** — они ещё не существуют! pip при установке компилирует байт-код (13.2) и дописывает строки `demo_hood/__pycache__/cli.cpython-312.pyc,,` (без хеша — pyc воспроизводим из исходника) в RECORD **установленной** копии.
...
## 18.7. pip изнутри: что происходит при `pip install` { #18.7 }
...
pip install demo-pkg-hood
  ...
  3. BUILD     если sdist — собрать wheel через PEP 517 (18.4); wheel кэшируется
  4. INSTALL   распаковка в site-packages → компиляция pyc (13.2) →
...
Три следствия, объясняющие классические «а почему так»: (1) shebang абсолютный — скрипт, положенный в venv, останется работать только с ним; (2) пути в RECORD относительные (`../../bin/demo-hood`) — pip uninstall удалит скрипт даже из соседнего каталога; (3) `python -m pip` (11.31) надёжнее голого `pip` именно потому, что гарантирует «тот pip, что принадлежит этому интерпретатору», — `bin/pip` мог остаться от другого окружения.
...
## 18.8. Editable-установка: `.pth` и finder { #18.8 }
...
`site` (11.31) при старте читает `.pth` и добавляет путь в `sys.path` — дальше работает обычный PathFinder (16.3). Проверка подтверждает: `demo_hood.__file__` указывает в `.../demo-pkg/src/demo_hood/__init__.py`, правки в исходниках видны без переустановки.
...
`.pth`-строка здесь **исполняет код при старте каждого Python** (вот и живое применение «`.pth` может выполнять arbitrary code» из 11.31) — finder встраивается в `sys.meta_path` (16.3) и отдаёт spec'и по явной карте `имя_пакета → каталог`. Импорты идут мимо обычного PathFinder, ровно и без побочных путей.
...
## 18.9. Зависимости: PEP 508 и PEP 440 { #18.9 }
...
Все три части проверены живыми прогонами; снаружи — это то, что лежит в `dependencies` (18.3) и `Requires-Dist:` в METADATA (18.2).
...
Отсюда разделение труда: `pyproject.toml` — декларация (диапазоны, маркеры, extras), `requirements.txt` — снимок окружения (по традиции жёстко закреплённые `==` версии; хеши — для репродьюсибл-установки, см. 11.31). Между ними стоит **lock-файл**: `pip freeze > requirements.txt`, `pip-tools` (`pip-compile`: резолвит и фиксирует весь транзитивный граф), `uv lock` (быстрый резолвер + `uv.lock`). Все решают одну задачу — детерминированная установка того, что декларативно задано диапазонами.
...
## 18.10. entry points: console-скрипты и плагины { #18.10 }
...
Секция `console_scripts` обрабатывается pip'ом при установке особым образом: для каждой записи пишется исполняемый файл в `bin/` (код обёртки — в 18.7). Функция **не импортируется до запуска команды** — обёртка делает `from demo_hood.cli import main` в момент вызова. Поэтому `pip install` быстрый, а первая команда ещё платит за импорт пакета (16.11 покажет его цену через `-X importtime`).
...
im.entry_points(group="demo_hood.plugins")   # наша группа из 18.3
...
## 18.11. Публикация: PyPI, twine и альтернативные формы { #18.11 }
...
**B3. Сборка wheel демо-проекта** (`pip wheel . --no-build-isolation`, setuptools 82): 1.72 с. В изоляции добавляется скачивание setuptools в окружение сборки — на холодном кэше +2–4 с. Это цена, которую платит каждый пользователь, ставящий sdist без wheel'а: вот почему авторам стоит выкладывать wheels (18.5).
...
| `im.entry_points(group="console_scripts")` — скан 430 dist-info | 15.3 мс |

Вывод для плагинных систем: точечный запрос — микро-цена, полный скан — миллисекунды; кэшируйте `entry_points()` на процесс и не дёргайте его в горячих путях (18.10).
