...
#   3           2 LOAD_FAST                0 (x)
#               4 LOAD_CONST               1 (0)
#               6 COMPARE_OP              68 (>)    # 68 = индекс `>` в таблице сравнений
#              10 POP_JUMP_IF_TRUE         7 (to 26)   # если условие ИСТИННО —
#                                                         прыгаем мимо raise-блока
#              12 LOAD_ASSERTION_ERROR                 # иначе готовим исключение
#              14 LOAD_CONST               2 ('bad')   # сообщение
#              16 CALL                     0           # вызов AssertionError('bad')
#              24 RAISE_VARARGS            1
#          >>   26 RETURN_CONST             0 (None)   # выход из функции
```
...
### PEP 657: тонкие локации ошибок в трейсбеках (3.11+) { #13.1-pep-657 }
...
Запуск через `python -m unittest` — это тот же runpy-механизм, что у `python -m json.tool` (11.32): интерпретатор импортирует пакет `unittest` как `__main__`, тот подхватывает `unittest.main()` и дальше — loader/runner. Живой вывод:
...
### Порядок тестов и discovery { #13.2-poryadok }
...
Это самое заметное отличие от pytest, который собирает голые функции `test_*` как полноценные тесты (13.6).

### TestResult: куда деваются skip, xfail и subTest { #13.2-result }
...
## 13.3. `unittest.mock` изнутри: анатомия подмены { #13.3 }
...
### Авто-атрибуты и запись вызовов { #13.3-zapis-vyzovov }
...
`side_effect` имеет три варианта поведения (проверяются в указанном порядке):

- **Функция** → вызывается, результат используется как возвращаемое значение (если не `DEFAULT`, иначе берётся `return_value`).
- **Исключение** (класс или инстанс) → поднимается.
- **Iterable** → каждый вызов берёт следующий элемент; если элемент — исключение, оно поднимается, иначе возвращается.
...
### spec и autospec: мок, который не врёт { #13.3-spec-i-autospec }
...
### `patch`: setattr с откатом — и классическая ловушка { #13.3-lovushka-from-import }
...
## 13.4. Подмена времени и окружения в тестах { #13.4 }
...
Уровень один — тот же `patch` из 13.3:
...
Часовые пояса подменяются окружением: `os.environ["TZ"] = "UTC"; time.tzset()` — POSIX-only (на Windows `tzset` нет), поэтому в кросс-платформенных тестах это делают через `monkeypatch`/`patch.dict` (2.4, 13.3) и пропускают тест на Windows (`@unittest.skipIf(sys.platform == "win32", ...)`).

## 13.5. `doctest` изнутри { #13.5 }
...
**Сравнение — посимвольное сравнение строк вывода (включая REPL-echo через `repr` для выражений без `print`) с ожидаемым текстом.** Если функция печатает `'abab'` через `print`, ожидание должно быть `abab` (без кавычек); если же это просто значение выражения в REPL — `'abab'` с кавычками (как `repr` вернёт). Отсюда же требование «пустая строка в ожидаемом выводе = конец примера»: парсер просто не знает, где вывод заканчивается.
...
⚠️ Границы применимости: doctest — это **документация с гарантией свежести**, а не набор тестов. Он идеален там, где пример и есть спецификация (README, docstring чистых функций), и плох там, где нужны фикстуры, изоляция состояния и проверка побочных эффектов — это уже территория 13.2/13.6.

## 13.6. `pytest` изнутри { #13.6 }
...
### Сбор: голые функции, TestCase, parametrize { #13.6-sbor }

Коллектор pytest обходит дерево от rootdir, импортирует файлы `test_*.py`/`*_test.py` и собирает всё, что похоже на тест: голые функции `test_*`, методы `test_*` в классах `Test*` (без `__init__`) — и заодно полноценные `unittest.TestCase` (13.2) в режиме совместимости:
...
def test_bare():                     # голая функция — pytest соберёт,
    assert 1 + 1 == 2                # unittest.discover — нет (13.2)
...
`parametrize` — это генерация тестов **на этапе сбора**: из одного определения делается три теста с идентификаторами `[2-4]`, `[3-9]`, `[4-16]` (значения параметров в имени). Статусы совпадают с unittest-овскими по смыслу, но есть важная разница в сборе: pytest собирает голые функции, unittest — нет (13.2).

### Assertion rewriting: почему assert pytest'а такой умный { #13.6-assertion-rewriting }

Обычный `assert a == b` в падении говорит только «assert failed» — выражение уже вычислено и потеряно (13.1). pytest перед импортом каждого тестового модуля **переписывает его AST**: каждый `assert` заменяется на код, который вычисляет подвыражения, сохраняет их в служебные переменные `@py_assert*` и в случае падения собирает из них человекочитаемое сообщение. Механику видно напрямую — тот же трансформер, что крутится в импорт-хуке:
...
Проверки строятся из того же сравнения `==`, что и обычный assert, но с сохранёнными operand'ами. Переписывание можно выключить (`--assert=plain`) — и вы сразу вернётесь к сухим сообщениям из 13.1.

### Фикстуры: dependency injection по имени { #13.6-fikstury }
...
## 13.7. `coverage.py`: измерение покрытия { #13.7 }

Coverage отвечает на вопрос «какие строки исполнились», и это тоже интерпретаторный крючок: чтобы узнать, на каких строках побежал код, нужно получать line-события трейсера (13.8). `coverage.py` — эталонная реализация, под капотом которой два переключаемых движка (ниже).
...
### Ветвное покрытие { #13.7-vetvnoe-pokrytie }
...
### Внутри: SQLite и движки трассировки { #13.7-dvizhki }
...
- `ctrace` (по умолчанию на 3.12, начиная с coverage.py 7.10) — C-трассировщик `CTracer`, собственной механикой повторяющий `sys.settrace` (13.8), но целиком в C: события line/call/return снимаются с минимальной питоновской обвязки;
- `sysmon` — `sys.monitoring` (PEP 669, 13.9): `COVERAGE_CORE=sysmon coverage run …`; неверное значение бракуется сразу: `Unknown core value: 'bogus'`.
...
## 13.8. `sys.settrace`/`sys.setprofile`: крючки отладчика и профайлера { #13.8 }

`pdb` (1.16), `bdb` (11.31), `profile` (11.31) и line-события coverage (13.7) сидят на одном и том же крючке — `sys.settrace`. Это функция-трейсер, которую интерпретатор вызывает при трёх классах событий:
...
Ключевое слово — **двухуровневость**: глобальный трейсер получает только `call` и своим **возвращаемым значением** назначает локальный трейсер для кадра:
...
Экономнее на порядки — потому профайлеры (`cProfile`, 11.31) используют именно `setprofile`, а отладчики — `settrace`.

### Что изменилось в 3.12: settrace теперь эмуляция { #13.8-izmeneniya-3-12 }

В 3.12 `settrace`/`setprofile` внутренне **переписаны поверх `sys.monitoring`** (13.9): это уже не прямой механизм eval loop, а адаптер (в исходниках CPython — `Python/legacy_tracing.c`). Появились поведенческие отличия, каждое из которых проверено на 3.12.14 и 3.13.5:
...
- **`opcode`-события через `frame.f_trace_opcodes = True`: на 3.12 не приходят** (эмуляция не раздаёт opcode-события из `settrace`-адаптера), на 3.13.5 — приходят. Если нужны инструкции — надёжнее `sys.monitoring` с `INSTRUCTION` (13.9).
...
Параноидальное дополнение из 11.31 остаётся в силе: если ваш код исполняет чужой Python и разрешает `breakpoint()`/`sys.settrace` — вы дали чужому коду полный контроль над выполнением. Песочницы перезаписывают `sys.settrace` первым делом.

## 13.9. `sys.monitoring` (PEP 669, 3.12+): один API для отладчиков, профайлеров и coverage { #13.9 }
...
(`OPTIMIZER_ID=5` зарезервирован — в 3.13 на него встал экспериментальный tier-2 оптимизатор из 15.5.)
...
### Локальные события: подписка на один код-объект { #13.9-lokalnye-sobytiya }
...
Кто уже сидит на PEP 669: coverage.py — опционально через `COVERAGE_CORE=sysmon` (13.7), фреймворки отладчиков (`debugpy` и наследники pdb) мигрируют постепенно, а в 3.14 на этот же API легла remote-отладка (PEP 768 — подключение отладчика к работающему процессу; его CLI-ручка `-X disable_remote_debug` разобрана в 10.3). Старый `settrace` (13.8) теперь — обёртка над этим же механизмом, так что у интерпретатора остался один системный крючок вместо трёх.

## 13.10. Тесты самого CPython: `python -m test` { #13.10 }

Внутри каждого CPython лежит `test` — пакет с ~сотнями тестовых модулей, и это не «какие-то тесты»: для альтернативных реализаций они играют роль исполняемой спецификации языка (17.1) — «прошёл test suite» — главное заявление о совместимости. Запускаются они тем же интерпретатором через regrtest (regression test runner):
...
Тот же runpy-механизм, что и у `-m unittest` (13.2): `test` — обычный пакет, `__main__` которого вызывает `test.libregrtest`. Полезные ручки (из `python -m test --help`):
...
| `-r` (`--randomize`) + `--randseed=N` | рандомизация порядка тестов (ловля скрытых зависимостей между тестами); `--randseed` фиксирует зерно для воспроизводимости |
...
### Охота на рефлаки: debug-сборка { #13.10-refleaki }
...
## 13.11. Тестовые режимы интерпретатора: шпаргалка { #13.11 }
...
### Бенчмарки к Части XIII { #13.11-benchmarki }
...
Покрытие — это трассировка всего кода, поэтому замедление в разы — норма; экономия sysmon (13.7, 13.9) на 3.12+ уже заметна и растёт на ветвистом коде.
