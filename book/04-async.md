...
⚠️ **Кооперативная многозадачность**: в `asyncio` задача выполняется до тех пор, пока сама добровольно не отдаст управление через `await`. Если внутри корутины вызвать синхронную операцию (`time.sleep(5)`, `requests.get()`, долгий CPU-расчёт), **зависнет весь Event Loop** — остановятся все корутины.
...
async def handler():
    ...
    try:
        ...
        # Если хочешь дождаться — передай task дальше или await task в finally.
        raise
