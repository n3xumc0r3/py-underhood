...
### Мёртвый код (dead code insertion)
...
### `setattr` для динамических атрибутов
...
### `types.FunctionType` + `types.CodeType` — сборка функции из байт-кода
...
### `type()` — динамическое создание классов
...
### `sys._getframe` — чтение локальных переменных вызывающего
...
### `ctypes.pythonapi.PyFrame_LocalsToFast` — запись изменений обратно
...
### Чтение данных из фрейма тестирующего
...
### Универсальный «комбайн» — собрать всё в JSON
...
### Цепочка: `().__class__.__base__.__subclasses__()`
...
### Универсальный «finder» — найти любой модуль в `__subclasses__`
...
### Защита: почему это сложно заблокировать
...
### Эффективность приёмов против разных систем
...
### Зелёная зона — что легально и эффективно
