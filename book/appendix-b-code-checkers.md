...
## B.1. Косметические приёмы — обман на уровне токенов { #B.1 }
...
⚠️ Constant folding (свёртка констант) на этапе компиляции сворачивает `1 + 1 + 1` обратно в `3` в байт-коде. Но антиплагиат-системы работают с **исходником**, не с байт-кодом — для классического Winnowing без normalisation-слоя (A.1) `1+1+1` и `3` — разные токены. Современные системы (A.8) перед сравнением запускают собственные оптимизации и могут различать их только если folding отключён — это вопрос конфигурации конкретного инструмента.

## B.2. Архитектурные приёмы — перенос логики в рантайм { #B.2 }
...
code = compile("lambda x: x * 2 + 1", "<dynamic>", "eval")
func = types.FunctionType(code.co_consts[0], globals(), "f")
# ⚠️ FunctionType нужен CODE-объект: у eval-строки "x * 2 + 1" co_consts[0] —
# это int, а не код; оборачиваем в lambda — у неё co_consts[0] — код тела
...
AST видит: `AnnAssign` + `Import` + `Call` (специального узла `ast.Exec` в Python 3 нет — `exec(...)` это обычный вызов функции). Что реально выполняется — не видно без запуска.

⚠️ **Все архитектурные приёмы детектируются динамическим анализом** (A.7): запустите код, соберите trace, сравните поведение. Но динамический анализ дорог и редко используется в тестирующих системах.

## B.3. Перехват аргументов через фреймы и ctypes { #B.3 }
...
def check_solution(student_answer):
    ...
    # ... код проверки ...
    result = read_frame_vars()  # ← чтение expected_answer из фрейма check_solution
...
⚠️ `PyFrame_LocalsToFast` — внутренняя C-API функция (упомянута в C-API документации, но без гарантий стабильности). Работает на CPython, не работает на PyPy/Jython. Может быть удалена в будущих версиях.
...
## B.4. Интроспекция окружения тестирующих систем { #B.4 }

Интроспекция окружения — запустить код в тестирующей системе и собрать **всю** информацию об окружении через один `raise`. Результат попадает в stderr, который система обычно показывает при ошибке.
...
## B.5. Песочницы и обход через `__subclasses__()` { #B.5 }
...
### Универсальный «finder» — найти любой модуль в `__subclasses__` { #universalnyy-2 }
...
### Защита: почему это сложно заблокировать { #zaschita-2 }
...
## B.6. Что работает, а что нет — реалистичная оценка { #B.6 }
...
- **Мёртвый код в больших количествах** — цикломатическая сложность (A.4) растёт, метрики Хальстеда (A.3) аномальны.
