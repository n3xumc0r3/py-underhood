...
`target-version = "py312"` — ключевой параметр для группы `UP`: при `py39` ruff предложит `from __future__ import annotations`, при `py312` — `type`-алиасы и другие новинки. Без этой настройки `UP` работает по консервативному минимуму.
