...
24. **Исходники CPython** на GitHub: https://github.com/python/cpython
    ...
    - `Objects/object.c` + `Objects/typeobject.c` — реализация `__eq__`/`__hash__`/`__repr__` для `object` (сами слоты dunder'ов — в `typeobject.c`) — база для понимания дескрипторов и `__class__` lookup (Часть VI)
...
| Дженерики через `[...]` (`list[int]`, PEP 585) | PEP 585 (№29) |
...
- **Fluent Python** Лючиано Рамальо (№22) — для современного Python (3.10+ — `match/case`, `zip(strict=True)`, протоколы, дескрипторы, метаклассы; t-строки 3.14 в издании 2022 года ещё не могли быть).
