...
Имя при этом не перезаписывается (`name = "tags"` в обоих классах), но изменяемое состояние (`default`, кэши внутри поля) становится общим. Отсюда и защита от повторной привязки в начале примера: привязать то же поле ко **второму** классу нельзя — Django на этом месте поднимает `FieldError`. Честное решение — клонировать поля в подклассах через `__init_subclass__` (см. 6.8) или не хранить изменяемое состояние в дескрипторе вовсе.
...
→ **см. также:** 5.6 — сам `__init_subclass__`; 6.7 — поля ORM; Часть VII (7.4) — метаклассы как тяжёлая артиллерия.
...
- **self участвует в ключе кэша** → каждый экземпляр удерживается в кэше, пока живёт метод: `Point` без `__weakref__` нельзя собрать, пока он в кэше. При `maxsize` ограниченно это управляемо; на большом потоке экземпляров — утечка по памяти.
- **`self` обязан быть hashable** — slots-класс хешем не обзаведётся сам (5.8): нужен `__hash__` из полей, иначе `TypeError` при первом же вызове.
