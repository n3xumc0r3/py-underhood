...
## 18.1. Три пути в C: `ctypes`, `cffi`, C API { #18.1 }
...
| Цена вызова (18.11) | ~400 нс | ~300 нс | ~90–120 нс | ~60–125 нс |
| GIL-контроль | `CDLL`/`PyDLL` | освобождается | свой код | свой код |
| Stable ABI | — | — | да | опционально (18.9) |
...
### Прототип обязателен: две ловушки `argtypes`/`restype` { #18.2-restype }
...
### Структуры, массивы, указатели { #18.2-structs }
...
### Колбэки: C вызывает Python { #18.2-callbacks }
...
⚠️ Колбэк из C в Python — самый дорогой вид вызова (FFI-переход в обе стороны, ~540 нс, 18.11) и самый опасный: если хранить ссылку на `cmp_func` нигде, а C-код держит указатель — GC соберёт обёртку, и следующий вызов из C упадёт. Живите на колбэке всё время, пока он нужен C-стороне.

### `CDLL` против `PyDLL`: кто держит GIL { #18.2-gil }
...
## 18.3. `cffi` — объявления вместо прототипов { #18.3 }
...
API-режим собирает C-файл, в котором ваши Python-вызовы `C.add2(...)` превращаются в прямые вызовы C с проверкой типов на этапе **компиляции** — граница FFI исчезает, остаётся обычный вызов функции расширения (18.11: на одном порядке с METH_*). Именно поэтому у `cryptography` нет бинарных проблем «ctypes-строка не совпала с реальной сигнатурой».

✅ PyPy-симметрия (17.2): на PyPy `cffi`-модули исполняются **нативно** JIT'ом (cffi — часть его stdlib), а C-расширения через мост `cpyext` теряют скорость на каждой границе. Если библиотеку планируется гонять на нескольких реализациях Python — API-режим cffi безопаснее C API.

## 18.4. `PyObject` и подсчёт ссылок { #18.4 }

Каждый объект CPython начинается с одного и того же заголовка. Упрощённо (без `_PyObject_HEAD_EXTRA`, пустого в release-сборках, но непустого в debug с `Py_TRACE_REFS`), базируется на `object.h` из CPython 3.12:
...
Весь Python — миллионы объектов списков, функций, модулей — сидит на этих двух полях: **сколько ссылок** и **какой тип**. «Класс» в рантайме — это `ob_type`, указывающий на `PyTypeObject` со слотами (18.6). Отсюда же прозрачность объектов для `ctypes`: заглянуть в заголовок можно без единой строчки C:
...
### Правила ссылок: owned vs borrowed { #18.4-refs }
...
### Бессмертные объекты (PEP 683, Python 3.12+) { #18.4-immortal }
...
## 18.5. Свой модуль на C API { #18.5 }
...
Суффикс `EXT_SUFFIX` — не украшение: именно по нему импорт-механика (16.3, 16.9) отличает расширение от исходника и проверяет совместимость ABI (`cpython-312` = только 3.12). Файл с суффиксом `.so` без тега тоже импортируется, но его нет в wheel-тегах (17.1).
...
### Конвенции вызова: `METH_*` { #18.5-conventions }
...
`METH_FASTCALL` — самый быстрый вариант для фиксированной арности: кортеж не создаётся вообще (18.11: 84 нс против 124 нс у VARARGS). Уточнение для stable ABI: в ограниченном API `METH_FASTCALL` доступен только с версии 3.10 (`Py_LIMITED_API >= 0x030A0000` — проверено по `methodobject.h` 3.12).
...
### Разбор аргументов: `PyArg_ParseTuple` { #18.5-parse }
...
### `PyModuleDef`: `m_size` и multi-phase init { #18.5-moduledef }

Поле `m_size` — не «размер», а флаг изоляции: `-1` — модуль держит глобальное состояние и суб-интерпретаторы (18.10) не поддерживает; `0` — состояние только в атрибутах модуля; `> 0` — статическая память на каждый суб-интерпретатор. Пойманный при написании этой главы `SystemError` — заодно демонстрация:
...
Multi-phase init (создание модуля отделено от исполнения `m_exec`) обязателен для free-threaded сборок (18.9) — там глобальное состояние недопустимо в принципе.

## 18.6. Свой тип из C API { #18.6 }
...
### Heap-типы: `PyType_FromSpec` { #18.6-heaptypes }

`static PyTypeObject` — глобальная C-переменная: тип нельзя создать динамически и нельзя выгрузить. Альтернатива — **heap-тип** из спецификации, доступный в том числе в ограниченном API (18.9):
...
## 18.7. Ошибки и исключения из C { #18.7 }
...
### Протокол нарушен: что бывает за `return NULL` без `PyErr_*` { #18.7-badnull }
...
## 18.8. GIL из C-кода { #18.8 }

GIL — «один eval loop на процесс» (15.5), конкурентность против параллелизма (4.10) — но на C-уровне это просто мьютекс, **владельцем которого является C-код расширения**, и он может его отдавать. Макрос-пара для CPU-bound/блокирующих участков:
...
### `PyGILState_*`: когда поток чужой { #18.8-pygilstate }
...
Противоположность — `PyDLL` из ctypes (18.2): он **не** освобождает GIL при вызове, потому что его функции вправе звать Python C API. Проверка на живой машине — тот же эксперимент, что выше: `CDLL`-сон не мешает главному циклу (3 017 607 итераций за секунду), `PyDLL`-сон блокирует всё (15 064).

Устаревший сахар: `PyEval_InitThreads()`/`PyEval_ThreadsInitialized()` deprecated с 3.9, удалены в 3.13 (см. changelog 3.13 и PEP 684 про per-interpreter GIL) — GIL инициализируется всегда, звать ничего не нужно.

## 18.9. Stable ABI (PEP 384) и сборки без GIL (PEP 703 / PEP 779) { #18.9 }

### Ограниченный C API: один `.so` на все версии { #18.9-limited }

Обычное расширение слинковано с внутренностями конкретной версии: заголовок 3.12 описывает поля структур, которые в 3.13 изменились. Определение `Py_LIMITED_API` перед `Python.h` (механика стандартизована PEP 384, 3.2+) отрезает всё нестабильное:
...
| Создание типов | static `PyTypeObject` | **обязательно** `PyType_FromSpec` (18.6) |
| Суффикс файла | `.cpython-312-x86_64-linux-gnu.so` | **`.abi3.so`** — грузится любой CPython ≥ 3.10 |
| Wheel-тег | `cp312-cp312-manylinux...` | `cp312-abi3-manylinux...` — один wheel на все версии (17.1) |
...
### Сборки без GIL { #18.9-freethreading }

| Сборка | С | Что для расширений |
|---|---|---|
| Классическая, один GIL | всегда | статус-кво этой части |
| Per-interpreter GIL (PEP 684) | **3.12+** | каждый суб-интерпретатор (18.10) — свой GIL; требует multi-phase init (18.5) и `m_size != -1` |
...
Free-threaded сборка — отдельный бинарник интерпретатора; обнаружить из C можно по макросу `Py_GIL_DISABLED`, из Python — `sys._is_gil_enabled()` (3.13+). Требования к расширению там радикально выше: разделяемая C-статика, лениво инициализированные глобальные указатели, кэши без блокировок — всё становится гонкой. Поэтому же multi-phase init (18.5) в free-threaded сборке обязателен.

## 18.10. Встраивание интерпретатора в C { #18.10 }
...
⚠️ Жизненный цикл жёстче, чем кажется: после `Py_FinalizeEx` повторная инициализация в том же процессе **не поддерживается**; из-за бессмертных объектов (18.4) часть памяти не возвращается ОС в принципе. Утилизаторам памяти стоит мерить «до finalize», а не RSS после выхода.

### Суб-интерпретаторы { #18.10-subinterps }

В одном процессе можно держать несколько изолированных интерпретаторов — `Py_NewInterpreter()`/`Py_EndInterpreter()`, в 3.13 появился публичный модуль `interpreters` (PEP 734, всё ещё provisional); ранее доступ был только через `_interpreters`. Каждый суб-интерпретатор имеет собственные `sys.modules` и поток-стейты; с 3.12 (PEP 684) каждый может получить и **свой GIL** — это фундамент будущего параллелизма внутри одного процесса. Условие из 18.5 всплывает здесь же: модуль с `m_size = -1` суб-интерпретаторы ломает, и при импорте в новый интерпретатор такие модули переинициализируются с нуля. Для честной изоляции модули должны хранить состояние в самом объекте модуля, а не в C-статике.

Практическая альтернатива, покрывающая 90% хотелок «встроить Python» — отдельный процесс: `subprocess` (11.30) или сокет к обычному интерпретатору; встраивание оправдано, когда нужен именно in-process вызов с нулевой задержкой границы.

## 18.11. Отладка C-расширений { #18.11 }
...
| `-X perf` (11.7, 15.5) | профилирование C-функций расширения в perf | `python3 -X perf script.py` + `perf record` |
...
### Бенчмарки к Части XVIII { #18.11-benchmarki }

Машина: 2 ядра, CPython 3.12.14, gcc 17.2, `timeit` best-of-5. Абсолютные числа — свойство машины; интересны отношения.
...
**B2. GIL release, 2 потока CPU-bound, N = 4 000 000** — см. таблицу в 18.8: 1.00× / 1.33× (released) / 2.00× (held).
