...
Снаружи Python 3 прост: весь текст — Unicode-`str`, а байты живут отдельно. Под капотом — директивы парсера, таблица кодеков, `search_function` и системные настройки, из-за которых один и тот же скрипт на разных машинах ведёт себя по-разному. Часть идёт от PEP 263 через собственный кодек (9.5) к механике поиска кодеков в `encodings/__init__.py` (9.6).
