...
✅ Практическое правило: sdist обязателен к публикации (PyPI требует; имя файла нормализуется по PEP 625; некоторые упаковщики/линукс-дистрибутивы собирают только из него), но добрые пакеты публикуют и wheel для каждой платформы. Если пакет публикует **только** sdist с C-кодом — его установка потребует gcc у каждого пользователя (или см. 18.9 про stable ABI: один `cp312-abi3`-wheel покрывает 3.12+).
...
Бэкенды различаются деталями: hatchling по умолчанию (mode = `compat`) пишет простой `.pth`, но поддерживает и более строгий mode = `strict` с custom finder; flit-core — всегда простой `.pth`; setuptools — как разобрано выше (две стратегии: `.pth` или finder). Поведение заметно, когда ловят «а почему мой модуль импортируется из корня проекта» — это у вас flat-layout + простой `.pth`-инструмент: добавьте `src/`.
...
im.entry_points(group="console_scripts", name="pip")   # параметр name= появился в 3.12; на 3.9–3.11 фильтруйте результат вручную
