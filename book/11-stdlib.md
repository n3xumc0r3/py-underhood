...
⚠️ **У `partial` нет `__name__`** (`__doc__` есть, но общий, не от оборачиваемой функции) — при передаче во фреймворки (Celery, Flask) используйте `functools.update_wrapper(p, p.func)`, он скопирует `__name__`, `__qualname__`, `__doc__`, `__module__`, `__annotations__`, `__type_params__` и обновит `__dict__` (через `WRAPPER_ASSIGNMENTS`/`WRAPPER_UPDATES`).
...
# cleanup НЕ вызовется немедленно (он будет отложен до выхода из процесса
# через atexit-хук weakref.finalize._shutdown). Передавайте вместо объекта
# его данные (obj.name, путь).
```
...
# - DeprecationWarning — для разработчиков (по умолчанию игнорируется, КРОМЕ триггеров из __main__ — Python 3.7+)
...
Регистрация через аннотацию типа: если `@to_json.register` вызвано без явного типа, берётся `__annotations__` первого аргумента функции. Имя `_` — лишь конвенция, чтобы не засорять namespace (но аннотация читается вне зависимости от имени функции).
...
**Иерархия логгеров** — `app.api.v1` **эффективный уровень** поднимается вверх (если у `app.api.v1` не выставлен — берётся от `app.api`, потом `app`, потом root). Записи **propagируются вверх** к родительским хендлерам (если `propagate=True`); хендлеры не «копируются вниз» — каждый логгер использует свой набор плюс родительские:
...
✅ Правило: `timedelta` прибавляется к полю даты/времени (wall clock), `tzinfo` остаётся как есть — зона не пересчитывается. Для «завтра в это же время» этого обычно и хотят; для «ровно через 24 часа абсолютных» — ходите через UTC. И да: `amb1 - amb0` из примера выше даст `0:00:00` — потому что `tzinfo` у обоих один и тот же объект.
...
$ python3 -m json.tool --json-lines data.jsonl    # парсит каждую строку как отдельный JSON (без флага падает на NDJSON); на выходе каждый объект форматируется отдельно
...
⚠️ **`BufferError: cannot close exported pointers exist`**: пока на mmap живёт `memoryview`, `mm.close()` упадёт — нельзя закрыть буфер, у которого «на руках» экспортированные указатели. Это самая частая авария zero-copy-работы: сначала `mv.release()` (или `del mv`), потом `close()`; либо держите memoryview внутри `with`-блока mmap. Срезы (способ 2) так не страдают — их копия `bytes` от mmap не зависит.
...
- **Синхронизация на вас** — `multiprocessing.Lock` или `mmap`+атомики. Без этого — race conditions.
...
| **macOS** | То же, что Linux, но `/dev/shm` нет — используйте `tempfile` или `multiprocessing.shared_memory`. |
...
Универсальный совет: если нужен IPC между процессами — используйте `multiprocessing.shared_memory` (Python 3.8+), он скрывает платформенные различия. Чистый `mmap` — для работы с большими файлами в одном процессе.
...
Когда передаёте **строку** с `shell=True`, Python вызывает `/bin/sh -c "ваша строка"`. Shell парсит строку по своим правилам — метасимволы `;`, `|`, `&`, `` ` ``, `$()`, `>`, `<` интерпретируются. Если в строке есть данные от пользователя — он может «дописать» команду:
...
Если `shell=True` неизбежен — **всегда** используйте `shlex.quote()` для пользовательских данных:
...
| `subprocess.Popen(...)` | интерактивный процесс: пишете в stdin, читаете stdout построчно, общаетесь с долгоживущим процессом. |
| `os.system(cmd)` | ❌ устаревший. Не возвращает вывод, не безопасен, только returncode. |
| `os.popen(cmd)` | ❌ устаревший. Используйте `subprocess.Popen` с `PIPE`. |
...
- `setup` выполняется один раз, не входит в замер. Используйте для импортов и подготовки данных.
...
- `timeit` **отключает GC** на время замера (`gc.disable()`/`gc.enable()`). Если ваш код создаёт циклы — это искажает реальную картину. Для реалистичной картины — включайте обратно руками.
...
⚠️ **`profile` vs `cProfile`**: всегда используйте `cProfile`, если нет специфических причин. `profile` — чистый Python, в нём накладные расходы на каждый вызов ~10× больше, чем измеряемая функция. `cProfile` на C, почти не влияет на скорость.
...
`code` — модуль для создания **своих интерактивных REPL**. На нём построены `code.interact`/`InteractiveConsole`, `python -m code` и многие встраиваемые консоли; сам `python -i` (C-уровневый REPL) и `IPython` (prompt_toolkit) — не на нём. Если вы делаете песочницу или встраиваемую консоль — это ваш фундамент.
...
Все четыре (`timeit`, `bdb`, `profile`/`cProfile`, `code`/`codeop`) — это **готовые движки выполнения Python-кода**. Если вы пишете песочницу:
...
2. **`bdb`** — через `set_trace()` ставит хук на **каждую** строку. Злоумышленник может прочитать локальные переменные, изменить их, пропустить проверку. Запретите `sys.settrace` и `breakpoint` в песочнице.
...
⚠️ **venv и системные пакеты**: `--system-site-packages` — **не рекомендуется**. Если системный `requests` обновится, ваш код сломается без предупреждения. Изолируйте полностью.
...
⚠️ **Никогда не запускайте `pip` с `sudo`** — это сломает системный Python. Всегда в `venv`.

⚠️ **`pip install` выполняет код build-backend** (а при наличии `setup.py` — и его) — пакет может запустить arbitrary code при установке. Ставьте только доверенные пакеты.
